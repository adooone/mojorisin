...
var is = {
    ...
    obj(a) { return stringContains(Object.prototype.toString.call(a), 'Object'); },
    pth(a) { return is.obj(a) && a.hasOwnProperty('totalLength'); },
    svg(a) { return a instanceof SVGElement; },
    inp(a) { return a instanceof HTMLInputElement; },
    dom(a) { return a.nodeType || is.svg(a); },
    ...
    col(a) { return (is.hex(a) || is.rgb(a) || is.hsl(a)); },
    key(a) { return !defaultInstanceSettings.hasOwnProperty(a) && !defaultTweenSettings.hasOwnProperty(a) && a !== 'targets' && a !== 'keyframes'; },
};
...
function parseEasingParameters(string) {
    ...
    return match ? match[1].split(',').map((p) => { return parseFloat(p); }) : [];
}
...
const penner = (function () {
    ...
    const curves = {
        ...
        InOut: [
            ...
            function (a, p) { return function (t) { return t < 0.5 ? elastic(a, p)(t * 2) / 2 : 1 - elastic(a, p)(t * -2 + 2) / 2; }; }, /* inOutElastic */
        ],
    };

    const eases = {linear: [0.250, 0.250, 0.750, 0.750]  };

    const loop = function (coords) {
        curves[coords].forEach((ease, i) => {
            eases[`ease${coords}${names[i]}`] = ease;
        });
    };

    for (const coords in curves) loop(coords);
...
function flattenArray(arr) {
    return arr.reduce((a, b) => { return a.concat(is.arr(b) ? flattenArray(b) : b); }, []);
}
...
function arrayContains(arr, val) {
    return arr.some((a) => { return a === val; });
}
...
function hexToRgba(hexValue) {
    ...
    const hex = hexValue.replace(rgx, (m, r, g, b) => { return r + r + g + g + b + b; });
...
function parseTargets(targets) {
    ...
    return filterArray(targetsArray, (item, pos, self) => { return self.indexOf(item) === pos; });
}
...
function normalizePropertyTweens(prop, tweenSettings) {
    ...
    return propArray.map((v, i) => {
        ...
        return obj;
    }).map((k) => { return mergeObjects(k, settings); });
}


function flattenKeyframes(keyframes) {
    const propertyNames = filterArray(flattenArray(keyframes.map((key) => { return Object.keys(key); })), (p) => { return is.key(p); })
...
function normalizeTweenValues(tween, animatable) {
    ...
    for (const p in tween) {
        ...
        if (is.arr(value)) {
            value = value.map((v) => { return getFunctionValue(v, animatable); });
...
const setProgressValue = {
    css(t, p, v) { return t.style[p] = v; },
    attribute(t, p, v) { return t.setAttribute(p, v); },
    object(t, p, v) { return t[p] = v; },
...
function getInstanceTimings(animations, tweenSettings) {
    ...
    const timings = {};
    timings.duration = animLength ? Math.max(...animations.map((anim) => { return getTlOffset(anim) + anim.duration; })) : tweenSettings.duration;
    timings.delay = animLength ? Math.min(...animations.map((anim) => { return getTlOffset(anim) + anim.delay; })) : tweenSettings.delay;
    timings.endDelay = animLength ? timings.duration - Math.max(...animations.map((anim) => { return getTlOffset(anim) + anim.duration - anim.endDelay; })) : tweenSettings.endDelay;
    return timings;
}
...
function handleVisibilityChange() {
    if (document.hidden) {
        activeInstances.forEach((ins) => { return ins.pause(); });
        pausedInstances = activeInstances.slice(0);
        activeInstances = [];
    } else {
        pausedInstances.forEach((ins) => { return ins.play(); });
    }
}
...
function anime(params) {
    ...
    let startTime = 0; let lastTime = 0; let
        now = 0;
    let children; let
        childrenLength = 0;
    let resolve = null;
    ...
    function makePromise(instance) {
        const promise = window.Promise && new Promise(((_resolve) => { return resolve = _resolve; }));
    ...
    function toggleInstanceDirection() {
        ...
        children.forEach((child) => { return child.reversed = instance.reversed; });
    }
    ...
    function syncInstanceChildren(time) {
        if (!instance.reversePlayback) {
            ...
            for (let i$1 = childrenLength; i$1--;) { seekCild(time, children[i$1]); }
        }
    }
    ...
    function setAnimationsProgress(insTime) {
        ...
        while (i < animationsLength) {
            ...
            if (tweenLength) { tween = filterArray(tweens, (t) => { return (insTime < t.end); })[0] || tween; }
...
function stagger(val, params) {
    ...
    return function (el, i, t) {
        ...
        if (!values.length) {
            ...
            if (easing) { values = values.map((val) => { return easing(val / maxValue) * maxValue; }); }
            if (direction === 'reverse') { values = values.map((val) => { return axis ? (val < 0) ? val * -1 : -val : Math.abs(maxValue - val); }); }
        }
